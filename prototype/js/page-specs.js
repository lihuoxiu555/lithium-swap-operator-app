  ...
  const SPECS = {
    ...
    workbench: S({
      ...
      stats: [
        ...
        { name: "电池快照", def: "总数/使用中/空闲；离线另计不进三格" },
      ],
      fields: [
        ...
        { name: "电池数据", def: "总数 / 使用中 / 空闲。标题点设备统计" },
        { name: "Mock", def: "三卡 86/18/4690；用户见 decision-055；电池 12/5/5" },
      ],
      enums: ["今日：[今日0点,明日0点)"],
    ...
    leaseSub: S({
      ...
      source: "APP 现场操作",
      fields: [
        { name: "次页", def: "按当前操作出：计费 / 支付 / 更换 / 合同 / 结束等" },
        { name: "结束订单", def: "电池状态；逾期费用正常/自定义；押金全扣/实退；无租金" },
      ],
      empty: "记录类无数据「暂无记录」",
      error: "实退押金或自定义逾期未填不可提交；逾期>0 提交 toast 提示骑手端待付",
    }),
    sites: S({
      title: "站点列表",
      source: "PC 一期站点信息",
