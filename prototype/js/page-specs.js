  ...
  const SPECS = {
    ...
    dataStats: S({
      ...
      fields: [
        ...
        { name: "经营统计", def: "今日三卡 + 用户数据 9 格；三卡点进经营统计详情。三卡下「点击查看近7日趋势」引导" },
        { name: "资产统计", def: "原设备统计概况（柜/电池/站点/仓口 + 按站点）；无顶部电池四格快照" },
      ],
      enums: ["今日：[今日0点,明日0点)"],
    ...
    "dataStats:asset": S({
      ...
      fields: [
        { name: "设备概况", def: "柜机/电池/站点/仓口 + 按站点（电池组含总数/在线/离线/在柜等）" },
      ],
    }),
    workbench: S({
      title: "工作台",
    ...
    "bizStats:charts": S({
      ...
      empty: "本期 Mock 有柱",
      note: "默认近7日。自然月模式才显 ‹ 年 › + 月份宫格。双柱同比随模式：今日=去年同日段，近7=去年同7日，自然月=去年同月。柱顶本期与同比各标一数。KPI 不随图表筛变。",
    }),
    "bizStats:busy": S({
      title: "经营统计 · 站点繁忙度",
      source: "PC 站点经营分析",
